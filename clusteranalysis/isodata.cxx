...
IsoData::IsoData( int kMin, int kMax, int minPoints, float terminationThreshold, float mergeThreshold,
...
int IsoData::Run()
{
	...
	if( space->GetDimension() == 0 )
		return 0;
	Coordinate terminationVector = 
		Coordinate::Singular( terminationThreshold, space->GetDimension() );
	Coordinate distortionDelta = 
		Coordinate::Singular( terminationThreshold + 1, space->GetDimension() );
	...
	Coordinate stdDevCur = Coordinate::Singular( maxDistance, space->GetDimension() );
	Coordinate stdDevPrev = Coordinate::Singular( maxDistance, space->GetDimension() );
	do {
		stdDevCur = Group();
		Eliminate();
...
int IsoData::Split()
{
	Coordinate thresholdVector = 
			Coordinate::Singular( splitThreshold,
						space->GetDimension() );
	list<Cluster *>::iterator clusterIter = clusterList.begin();
	while( clusterIter != clusterList.end() )
	{
